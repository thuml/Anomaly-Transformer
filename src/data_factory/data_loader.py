...
class DBSSegLoader(object):
    def __init__(self, data_path, win_size, step, mode="train"):
        ...
        self.test = self.scaler.transform(test_data)
        self.train = data
        self.val = np.load(data_path + "/val.npy")
        self.train_labels = np.load(data_path + "/train_label.npy")
        self.val_labels = np.load(data_path + "/val_label.npy")
...
def get_loader_segment(
    ...
    if dataset == "SMD":
        ...
        dataset = DBSSegLoader(data_path, win_size, step, mode)
    shuffle = False
    if mode == "train":
        shuffle = True
